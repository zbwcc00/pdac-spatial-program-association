# GitHub and Zenodo v1.0.7 archive-byte-consistency release procedure
...
only the reviewed v1.0.7 revision files. Do not add raw GEO archives, local
...
`release_verification.json`, and that all version labels read `v1.0.7`.
...
v1.0.7 content. Commit those edits, then create a versioned annotated tag and
push it:

```powershell
git add -A
git commit -m "Release v1.0.7 archive-byte-consistency correction"
git tag -a v1.0.7 -m "PDAC spatial program association v1.0.7"
git push origin main --tags
```

Create a GitHub Release from the `v1.0.7` tag. Attach no raw GEO data.

## 4. Archive with Zenodo and obtain the DOI

Sign in to [Zenodo](https://zenodo.org/), open the GitHub integration, and
confirm that the public repository is enabled. Creating the GitHub `v1.0.7`
...
Confirm that GitHub shows the `v1.0.7` tag, Zenodo resolves the DOI, GitHub
